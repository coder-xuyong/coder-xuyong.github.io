...
## BeanFacotry 入门
1. 自定义一个bean，配置到xml文件中；在idea中，new-> XML configuration file 创建**beans.xml**，添加bean内容
```xml
<bean id="demoName" class="com.study.demo"></bean>
```
2. 通过BeanFacory获取对象
```java
DefaultListBeanFacotry factory = new DefaultListBeanFacotry();//创建工厂对象
XmlBeanDefinitionReader reader = new XmlBeanDefinitionReader(facotry);//创建读取器（XML）
reader.loadBeanDefinitions("beans.xml");//读取配置文件
Demo dm = (Demo) facotry.getBean("demoName");//获取对象
```
