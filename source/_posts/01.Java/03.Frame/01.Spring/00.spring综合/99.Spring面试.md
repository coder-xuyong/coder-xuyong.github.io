...
**Spring IoC** 是指 Spring 框架中的**核心容器**（`ApplicationContext` 或 `BeanFactory`），它负责：

1. **管理 Bean 的定义**：扫描 `@Component`、`@Service`、`@Controller` 等注解，或解析 XML，生成 `BeanDefinition` 注册到容器。
2. **实例化对象**：利用**反射**（你之前问过的）调用构造器创建对象实例。
3. **装配依赖**：通过依赖注入（DI）将各个 Bean 关联起来。
4. **管理生命周期**：控制 Bean 的初始化、销毁、作用域（单例/原型等）。

**一句话描述**：**Spring IoC 容器就像一个巨大的“对象工厂”，它负责把所有对象造出来、组装好、管理好，你只需要从里面拿现成的用，再也不用手动 `new` 了。**
...
#### ⭐什么是 AOP？你项目里有使用到AOP吗？

AOP(Aspect-OrientedProgramming:面向切面编程)能够将那些与业务无关，却为业务模块所共同调用的逻辑或责任(例如事务处理、日志管理、权限控制等)封装起来，便于**减少系统的重复代码，降低模块间的耦合度，提高系统可拓展性和可维护性**。

使用AOP统一管理日志的实现：
- 自定义@Log注解，用于针对核心业务方法的日志打印，避免因查询接口流量过大导致接口把磁盘写爆。
- 编写AOP切面类,@Pointcut去锁定标记由@Log的方法，@Around环绕通知记录方法的耗时、入参和出参。
...
### ⭐事务的传播行为有哪些？
默认 REQUIRED（跟着走），日志独立 REQUIRES_NEW（另起炉灶），批量回滚用 NESTED（打标记点），查询尽量 SUPPORTS（非事务跑）。

### ⭐Spring事务什么时候会失效？
- 代理失效：同一个类里的 A 方法（有 @Transactional）调用 B 方法（有 @Transactional），B 的事务不生效。
- 异常没有回传给代理：事务切面在方法出口处检查是否有异常。如果异常被 catch 且没重新抛出，切面认为执行成功，正常提交。
...
- 主要是Spring Boot的启动类上的核心注解`SpringBootApplication`注解主配置类，有了这个主配置类启动时就会为SpringBoot开启一个`@EnableAutoConfiguration`注解自动配置功能。
- 自动装配核心功能的实现实际是通过 `AutoConfigurationImportSelector`类
- 有了这个`EnableAutoConfiguration`的话就会：
  a. 从配置文件`META_INF/Spring.factories`加载可能用到的自动配置类
  b. 去重，并将exclude和excludeName属性携带的类排除
  c. 过滤，将满足条件（@Conditional）的自动配置类返回
