...
- 在 Minor GC 中，`Eden` 区中仍然存活的对象会被移动到其中一个 `Survivor` 区（比如 `from` 区）；
- 同时，如果 `from` 区中已经有对象，那么 `from` 区中存活的对象也会被移动到 `to` 区；
- 然后，清空 `Eden` 区和 `from` 区，此时 `from` 区变成空的，而 `to` 区存放了从 `Eden` 和 `from` 区中存活下来的对象。
- 接下来， `from` 和 `to` 的角色会交换。也就是说，下一次 Minor GC 时，当前的 `to` 区将作为 `from` 区（即存活对象被移出的区），当前的 `from` 区将作为 `to` 区（即存活对象被移入的区）。

`Survivor` 区域是JVM随意选择的，然后会在 GC 过程中进行区域间拷贝，也就是将 Eden 中存活下来的对象和 `from` 区域的对象，拷贝到这个`to`区域。这种设计主要是为了防止内存的碎片化，并进一步清理无用对象。
