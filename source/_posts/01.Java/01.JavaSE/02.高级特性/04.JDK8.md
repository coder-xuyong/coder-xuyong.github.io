...
返回一个由此流的元素组成的流，同时在从生成的流中消费每个元素时执行所提供的操作。
这是一个中间操作。
对于并行流管道，该操作可能在上游操作提供元素的任何时间和任何线程中被调用。如果该操作修改共享状态，则必须提供所需的同步。
参数：action - 在从流中消费元素时对其执行的非干预性操作
返回值：新的流
API 注意：此方法主要为支持调试而存在，在调试时您希望查看流经管道中某一点的元素：
```java
...
api注释翻译：
```txt
返回一个流，该流由将此流中的每个元素替换为应用提供的映射函数后生成的映射流的内容组成。每个映射流在其内容放入此流后都会被关闭。（如果映射流为 null，则使用空流代替。）
这是一个中间操作。
参数：映射器 - 一种应用于每个元素的无干扰、无状态函数，可生成新值流
返回值：the new stream
API 注意：flatMap() 操作会对流中的每个元素应用一种一对多的转换，然后将结果元素展平为一个新的流。
    示例：
    如果 orders 是一个包含采购订单的流，且每个采购订单包含一个行项目集合，
    那么以下代码将生成一个包含所有订单中所有行项目的流：
       ` orders.flatMap(order -> order.getLineItems().stream())...`
    如果 path 是一个文件的路径，那么以下代码将生成一个包含该文件中所有单词的流：
        `Stream<String> lines = Files.lines(path， StandardCharsets.UTF_8)；   Stream<String> words = lines.flatMap(line -> Stream.of(line.split(" +")))；`
    传递给 flatMap 的映射器函数使用简单的正则表达式将一行拆分为单词数组，然后从该数组创建一个单词流。
```
**案例：嵌套集合展平**
