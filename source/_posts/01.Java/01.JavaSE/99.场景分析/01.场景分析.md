...
定位：
- 系统已经OOM挂了，提前设置`-XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=`（要保证磁盘空间），可以在服务宕机的时候导出dump文件到指定的路径中。通过VisualVM程序去查看这个文件，可以去看到实例数和占的大小；我们可以去看这个示例，看它的gc root，找到线程引用查看到具体的代码。
