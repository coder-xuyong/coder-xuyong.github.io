...
![并发框架层次](img/java-concurrent-basic-mechanism.png)
...
![Mark Word](img/20200629191250.png)

锁升级功能主要依赖于 Mark Word 中的**锁标志位**和是否**偏向锁标志位**，`synchronized` 同步锁就是从偏向锁开始的，随着竞争越来越激烈，偏向锁升级到轻量级锁，最终升级到重量级锁。
...
**当没有竞争出现时，默认会使用偏向锁。**JVM 会利用 CAS 操作（compare and swap），在对象头上的 Mark Word 部分设置线程 ID，以表示这个对象偏向于当前线程，所以并不涉及真正的互斥锁。这样做的假设是基于在很多应用场景中，大部分对象生命周期中最多会被一个线程锁定，使用**偏斜锁可以降低无竞争开销**。
...
![偏向锁的获得和撤销流程](img/20200604105151.png)
...
![轻量级锁及膨胀流程](img/20200604105248.png)
...
> 即时编译（Just-In-Time Compilation）：在Java中，JIT编译器是一种动态编译技术，它在程序运行时将Java字节码转换为本地机器代码，从而提高程序的执行效率。 
...
public static String concatString(String s1, String s2, String s3) {
    ...
    sb.append(s1);// 这里面有 synchronized 锁！
    sb.append(s2);// 这里面有 synchronized 锁！
    sb.append(s3);// 这里面有 synchronized 锁！
    return sb.toString();
}
```
...
**互斥同步最主要的问题是线程阻塞和唤醒所带来的性能问题**，因此互斥同步也被称为阻塞同步。互斥同步属于一种**悲观的并发策略**，总是认为只要不去做正确的同步措施，那就肯定会出现问题。

随着硬件指令集的发展，我们可以使用基于冲突检测的**乐观并发策略**：先进行操作，如果没有其它线程争用共享数据，那操作就成功了，否则采取补偿措施（不断地重试，直到成功为止）。这种乐观的并发策略的许多实现都不需要将线程阻塞，因此这种同步操作称为**非阻塞同步**。

为什么说乐观锁需要 **硬件指令集的发展** 才能进行？因为需要操作和冲突检测这两个步骤具备原子性。而这点是由硬件来完成，如果再使用互斥同步来保证就失去意义了。硬件支持的原子性操作最典型的是：CAS。

**CAS**（Compare-And-Swap）是一种**无锁**的原子操作，用于实现多线程环境下的同步机制。它是现代并发编程的基石。
...
```
要如何解决问题；① 可以给`withdraw`加个锁；② 使用 CAS 。
...
```
CAS的工作流程图如下：
![cas工作流程](img/juc_2.png)


此外，**CAS 必须借助 volatile 才能读取到共享变量的最新值来实现【比较并交换】的效果**。
无锁的效率要比加锁的效率更高：
- 无锁情况下，即使重试失败，线程始终在高速运行，没有停歇，而 synchronized 会让线程在没有获得锁的时候，发生上下文切换，进入阻塞。
- 但无锁情况下，因为线程要保持运行，需要额外 CPU 的支持，没有获取到的话，虽然不会进入阻塞，但由于没有分到时间片，仍然会进入可运行状态，还是会导致上下文切换。
...
多个线程需要同时使用一个非线程安全的对象（比如 SimpleDateFormat），你会怎么做？
...
public class SharedDemo {
    ...
    public void doSomething() {
        // 多个线程同时调用 sdf.parse() 会导致数据错乱、异常
        Date date = sdf.parse(dateString);
    }
}
```
传统正确做法（同步锁synchronized），加锁虽然解决了线程安全，但引入了**性能开销**和**线程争用**。在高并发下，线程们需要排队等待锁。

因此有了 ThreadLocal，以空间换时间。
...
`ThreadLocal` 常用于防止对可变的**单例（Singleton）变量**或**全局变量**进行共享。典型应用场景有：管理数据库连接、Session。
